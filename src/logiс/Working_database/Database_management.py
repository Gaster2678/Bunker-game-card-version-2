...
from ..Working_randoms_db.Random_carts import *
...
async def reload_bd():
    await create_bd.create_all()
    await test_bd.test_bd_Last_value(last_play_room_id=10,last_user_id=10)
    ...
    for a in z:
        await Jobs_bd.add_play_and_room(id_send= a, id_discord_user= 1)
        for s in i:
            await Jobs_bd.add_play(id_send=a, id_discord_user= s)
